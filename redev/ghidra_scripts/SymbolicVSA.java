...
public class SymbolicVSA extends GhidraScript {
    ...
    AddressSet getCodeAddresRange() {
        ...
        if (!bFind) {
            String msg = String.format("Failed to find code segment");
            println(msg);
    ...
    @Override
    public void run() { // m_block --> basic block provided by ghidra / exec block --> mem access
        program = state.getCurrentProgram(); // select a program
        listing = program.getListing(); // assembly code X86 / IR

        VSADataTypeManager dtMgr = VSADataTypeManager.getInstance(program); // initialise handler to selected program

        /* travese all functions */
        AddressSet codesegRng = getCodeAddresRange(); // get address range of program instructions
        if (codesegRng == null) {
            return;
        }

        FunctionIterator iter = listing.getFunctions(true); // initialise iterator on all functions in the program
        Address startVM = codesegRng.getMinAddress(); // lowest address of instructions
        Address endVM = codesegRng.getMaxAddress(); // highest address of instructions

        while (iter.hasNext() && !monitor.isCancelled()) { // loop all functions found . MOnitor?
            Function f = iter.next(); // set current function
            String fname = f.getName(); // set function name
            Address f_startVM, f_endVM; 

            f_startVM = f.getBody().getMinAddress(); // set lowest address of instructions in target function
            f_endVM = f.getBody().getMaxAddress(); // set highest address of intructions in target function 
            ...
            String info = String.format("Function: name: %s entry: %s", f.getName(), f.getEntryPoint()); // format string to be printed
            println(info); // print function details (name, entry point)

            FunctionSMAR smar = new FunctionSMAR(program, listing, f, monitor); // initialise symbolic memory access recording (SMAR)
            																	// 'monitor' checks if variable is still alive.
            smar.doSMARecording(); // run VSA algorithm

            Map<Long, Map<String, Set<String>>> smart = smar.getSMARTable(); 	// hashmap --> key: memory block identifier
            																	//		   --> value: hashmap --> key: register/memory location
            																	//							  --> value: set of possible values 
            if (smart.size() > 0) {
                println("SMARTable: " + smart.toString());
            }

            DataAccessAnalysis analyzer = new DataAccessAnalysis(smart); // initialise analyser on SMAR table

            Set<String> array_info = analyzer.inferArrayAccess(); // memory accessed in this function
            if (array_info.size() > 0) {
                println("Possible arrays: " + array_info.toString());
            }

            Map<String, List<Long>> struct_access = analyzer.inferStructAccess(); // structures accessed in this function
            if (struct_access.size() > 0) {
                println("Possible structs: " + struct_access.toString());
            }

            SystemVLinux64Parameter param_updater = new SystemVLinux64Parameter(program); // 64-bit parameter interpreter on function
            Set<String> params = param_updater.inferActiveParameters(smart); // paramters accessed in this function
            if (params.size() > 0) {
                println("Number of parameters inuse: " + String.valueOf(params.size()));
            }

            Map<String, DataType> vsa_structs = dtMgr.inferDataStructs(struct_access); // returns inferred data structures
            param_updater.updateParameters(f, vsa_structs); // update parameters
        }
    }
}
...
class SystemVLinux64Parameter {
    ...
    /**
     * RDI, RSI, RDX, RCX, R8, R9, XMM0Ã¢â‚¬â€œ7
     */
    public Set<String> inferActiveParameters(Map<Long, Map<String, Set<String>>> symbolic_memory_access_table) {
 ...
 * defines a domain
 */
class FunctionSMAR { // m --> member of the current cfg node 
    private final Program m_program; // program that current CFG node is in
    private final Listing m_listDB; // list of member instructions to current CFG node
    private final Function m_function; // function that current CFG node is in
    private TaskMonitor m_monitor; // monitors if task(VSA task?) is alive

    private Map<Address, ExecutionBlock> m_blocks; 	// Wrapper for Ghidra's basic block
    												// Map --> Key: starting address of code block
    												//	   --> Value: execution block wrapper a CFG node
    ...
    /**
     * Construct the Control Flow Graph (CFG) for all basic blocks
     */
    private void constructCFG() {
        ...
        try {
            ...
            CodeBlockModel blkModel = new BasicBlockModel(m_program); // code block vs m_block vs execution_block? all wrappers?
            AddressSetView addrSV = m_function.getBody(); // function's address range 
            CodeBlockIterator codeblkIt = blkModel.getCodeBlocksContaining(addrSV, m_monitor); // iterator for all basic code blocks

            while (codeblkIt.hasNext()) { // loop all basic code blocks to create execution block
                CodeBlock codeBlk = codeblkIt.next(); // set curr code block
        ...
        try {
            ...
            for (ExecutionBlock curSMARBlk : m_blocks.values()) { // loop all m_blocks
                /* find the next-blocks of current code-block */
                Set<ExecutionBlock> nxtSMARblks = new HashSet<>(); // set of next possible execution blocks (next CFG node)
                CodeBlock curCodeBlk = curSMARBlk.getCodeBlock();
                CodeBlockReferenceIterator di = curCodeBlk.getDestinations(m_monitor); // get addresses of possible next, live code block
                while (di.hasNext()) {
    ...
    public boolean doSMARecording() { // 
    	...
    	// prefix m represents member. 
        Address fentry = m_function.getEntryPoint(); // entry point of this CFG node
        ExecutionBlock firstBlk = m_blocks.get(fentry); // first assemebly code in this CFG node
        if (firstBlk == null) {
            throw new NullPointerException("Cannot get the first block");
        }

        /* Initialize the Machine state */
        X86Interpreter inpt = X86Interpreter.getInterpreter(); // initialise 64-bit interpreter
        MachineState init_state = MachineState.createInitState(inpt.getCPU()); 	// machine state: 2 hashmap for one for registers 
        																		// and one one for memory slots. Initially no mem slots.
        																		// initialise both hashmaps to V (base of lattice)
        firstBlk.setInitMachState(init_state); // set machine state for first block

        try {
            /* loop until no changes to symbolic state (symbolic state?) */
            ExecutionBlock smarBlk;
            while (true) {
                /* pick up a block which has Machine-state to run? */
                smarBlk = null;
                for (ExecutionBlock blk : m_blocks.values()) { // 
                    int nState = blk.getNumOfMachState(); 	// block might be visited more than once by different paths
                    										// separate machine state for each time 
                    boolean bDirty = blk.isSMRTDirty(); // machine state changed

                    if (nState > 0 && bDirty) { // short circuit to process this node again
                        smarBlk = blk;
                        break;
                    }
                }
                ...
                /* smarBlk != null */
                traverseBlocksOnce(smarBlk); // actual updating of valuesets
            }
        } catch (Exception e) {
            /* fixe-me: ignore current function */
...
class SMARBlock {
    ...
    public void doRecording(MachineState state) {
        ...
        while (iiter.hasNext()) { // loop instructions
            Instruction inst = iiter.next();
            boolean suc = m_inpt.doRecording(state, smart, inst);
        }

        if (m_smarTable.containsAll(smart)) { // tables same == not dirty
            m_dirtySMART = false;
        } else {
...
class ExecutionBlock { // CFG node may be more than 1 instruction
    ...
    public void runCFGOnce() {
        ...
        for (Iterator<MachineState> itor = m_MachState.iterator(); itor.hasNext();) {
            ...
            m_block.doRecording(mstate); // loop instructions and updates valuesets
...
class MachineState {
    ...
    public static MachineState createInitState(X86Processor cpu) {
        ...
        for (String reg : allRegs) {
            s.m_regs.put(reg, "V" + reg); // V --> base of lattice? (IDK?)
        }
...
class X86Interpreter extends Interpreter {
    ...
    public boolean doRecording(MachineState state, Map<Long, Map<String, Set<String>>> table, Instruction inst) {
        ...
        try { // switch case on number of operands to determine what is instruction
            if (nOprand == 0) {
                _doRecording0(inst);
            } else if (nOprand == 1) {
                _doRecording1(inst);
    ...
    private void _doRecording0(Instruction inst) {
        ...
        if (op.equalsIgnoreCase("nop")) {
            ...
            /* CBW/CWDE/CDQE: AX Ã¢â€ ï¿½ sign-extend of AL. */
            return;
        }
    ...
    /**
     * PUSHÃ‚Â r/m16; PUSHÃ‚Â r/m32; PUSHÃ‚Â r/m64; PUSHÃ‚Â r16; PUSHÃ‚Â r32; PUSHÃ‚Â r64; PUSHÃ‚Â imm8;
     * PUSHÃ‚Â imm16; PUSHÃ‚Â imm32; PUSH CS; PUSH SS; PUSH DS; PUSH ES; PUSH FS; PUSH GS;
     *
     * @param inst
     */
    ...
    private void _record2addsub(Instruction inst, char op) {
        ...
        if (m_OPRDTYPE.isRegister(oprd0ty)) { // operand0 is register
            Register rOprd0 = (Register) objs0[0]; // initialise register obj as operand0
            strVal0 = getRegisterValue(rOprd0); // get register value (format: <String> RAX + 4 , <String> RSP, <String> RAX + RBP + 12, etc

            if (m_OPRDTYPE.isRegister(oprd1ty)) { // operant1 is resgister
                /* sub reg, reg */
                Register rOprd1 = (Register) objs1[0]; // initialise register obj as operand1
                strVal1 = getRegisterValue(rOprd1); // get register value 

                if (op == '+') // determine operation
                    strRes = m_SymCalc.symbolicAdd(strVal0, strVal1); // add using values from each operand
                else if (op == '-') // '-' operation
                    strRes = m_SymCalc.symbolicSub(strVal0, strVal1); // subtract using values from each operand
                else
                    strRes = strVal0; // fix-me

                updateRegisterWriteAccess(inst.getAddress(), rOprd0, strRes); // tracking of data movement.

            } else if (m_OPRDTYPE.isScalar(oprd1ty)) { // operant1 is scalar 
                /* sub rsp, 8; */
                Scalar sOprd1 = (Scalar) objs1[0]; // initialise operand1 as scalar value
            ...
            } else { // operant1 is in memory (variable; memory location)
                /* others */
        ...
        } else {// if operant0 is in memory (variable; memory location)
            ...
            if (m_OPRDTYPE.isRegister(oprd1ty)) { // operant1 is register
    ...
    /**
     * SHLÃ‚Â r/m8, 1 SHLÃ‚Â r/m8**, 1 SHLÃ‚Â r/m8, CL SHLÃ‚Â r/m8**, CL SHLÃ‚Â r/m8, imm8
     *
     * @param inst
     */
    ...
    /**
     * SHRÃ‚Â r/m8,1 SHRÃ‚Â r/m8**, 1 SHRÃ‚Â r/m8, CL SHRÃ‚Â r/m8**, CL SHRÃ‚Â r/m8, imm8
     * SHRÃ‚Â r/m8**, imm8
     *
     * @param inst
     */
    ...
    /**
     * SALÃ‚Â r/m8, 1 SALÃ‚Â r/m8**, 1 SALÃ‚Â r/m8, CL SALÃ‚Â r/m8**, CL SALÃ‚Â r/m8, imm8
     * SALÃ‚Â r/m8**, imm8
     *
     * @param inst
     */
    ...
    /**
     * SARÃ‚Â r/m8, 1 SARÃ‚Â r/m8**, 1 SARÃ‚Â r/m8, CL SARÃ‚Â r/m8**, CL SARÃ‚Â r/m8, imm8
     * SARÃ‚Â r/m8**, imm8
     *
     * @param inst
     */
    ...
    private String _calcMemAddress(String oprd_exp, Object[] objs_of_MemOperand) {
        ...
        Object[] objs = objs_of_MemOperand; // object vs parts?
        String strValue, strAddress;

        if (objs.length == 1) { // if operand is only 1 entity i.e. register || exact location
            /* mov reg, [reg]; mov reg, [0x48000] */
            if (objs[0] instanceof Register) { // is register
                Register r = (Register) objs[0];

                /* get regiser value */
                strValue = getRegisterValue(r.getName());
                return strValue;
            } else if (objs[0] instanceof Scalar) { // is scalar (what is a scalar? != address?)
                Scalar s = (Scalar) objs[0];
            ...
            } else if (objs[0] instanceof GenericAddress) { // is address
        ...
        } else if (objs.length == 2) { // operand mde up of 2 entities i.e. base addr and offset.
            /*
            ...
            if (parts.length == 1) {
                ...
                strValue = getRegisterValue(r.getName());
                strAddress = m_SymCalc.symbolicMul(strValue, s.getValue()); // why multiply?
            } else {
...
/**
 * Encapsulate calculation for symbolic values Singleton mode
 */
class SymbolicCalculator {
    ...
    public String symbolicBinaryOP(String symbol0, char op, String symbol1) {
        ...
        if (elems0.length == 1) { // format element 
        ...
        /* calculate the result (e.g. (RSP + 4) + (RBP + 7) == RSP + RBP + 11 */
        String tmpS, newSymbol;
        long tmpV;

        if (op == '+' || op == '-') {
            tmpS = binaryOP(part0S, op, part1S); // combine symbolic parts 
            tmpV = binaryOP(part0V, op, part1V); // combine offsets
    ...
    private String binaryOP(String pure_symbol, char op, long value) {
        ...
        if (isZero(pure_symbol)) // no symbolic value
            pure_symbol = "";
        ...
        } else if (value == 0) { // if value (offset) is 0, no change to symbol
            if (op == '+') {
        ...
        } else { // set new symbol as <original symbol, value>
            if (op == '+') {
                newValue = value;
     ...
     * @return
     */
    public long binaryOP(long value0, char op, long value1) { // simply execute oprator on operants as values
        long res;
